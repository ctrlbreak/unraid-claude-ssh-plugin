...
echo ""
echo "  Read-only SSH user installed."
echo "    User:    $USERNAME (SSH key auth only, password locked)"
echo "    Filter:  $FILTER_SCRIPT $FILTER_VERSION (validates every command)"
echo "    Audit:   tail -f /var/log/syslog | grep claude-shell"
echo ""
echo "  Quick smoke (run from a workstation; add the SSH key first):"
echo "    ssh $USERNAME@\$(hostname) 'ls /mnt/user'        # allowed"
echo "    ssh $USERNAME@\$(hostname) 'rm /tmp/test'        # blocked"
echo ""
echo "  Full command/flag/redirect rules: docs/filter.md"
